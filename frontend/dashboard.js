// FINAL dashboard.js with full requested features
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showView(name,push=true){
    ...
    if(name==='savings'){ const def = (state.profile.salary && state.profile.salary.type) ? state.profile.salary.type:'monthly'; showSavings(def);}
  ...
  function updateProfileUI(){
    const salary = state.profile.salary?.amount||0;
    const totalExpenses = state.expenses.reduce((s,e)=>s+(e.type==='expense'?Number(e.amount):0),0);
    const remaining = salary?(salary-totalExpenses):state.profile.savedAmount||0;
    ...
    profileNameEl.textContent = state.profile.name||'User';
    profileEmailEl.textContent = state.profile.email||'Not provided';
    profileSalaryEl.textContent = salary?formatCurrency(salary):'-';
    profileModeEl.textContent = state.profile.salary?.type||'-';
    profileSavedEl.textContent = state.profile.savedAmount?formatCurrency(state.profile.savedAmount):'0';
  }
  ...
  function renderCategories(){
    ...
    state.categories.forEach(cat=>{
      ...
      nameSpan.className='cat-name'; nameSpan.textContent=capitalize(cat);
      ...
      expenseSpan.className='cat-expense'; expenseSpan.textContent=formatCurrency(totalCat);

      const addBtn=document.createElement('button');
      addBtn.className='cat-add'; addBtn.textContent='Add Item';
      addBtn.onclick=()=>{ openCategoryModal(cat); };
  ...
  function openCategoryModal(cat){
    ...
    modalSave.onclick=()=>{
      const e={category:modalCategory.value,amount:Number(modalAmount.value||0),type:modalType.value,date:new Date().toISOString()};
  ...
  addCatBtn.addEventListener('click', ()=>{
    ...
    if(val && !state.categories.includes(val)){ state.categories.push(val); customCatInput.value=''; persistState(); renderCategories(); showToast('Category added'); }
  });

  function renderModalCategories(){
    modalCategory.innerHTML='';
    state.categories.forEach(c=>{ const opt=document.createElement('option'); opt.value=c; opt.textContent=capitalize(c); modalCategory.appendChild(opt); });
  }
  ...
  window.editExpense=function(idx){
    const e=state.expenses[idx]; if(!e) return;
    ...
    modalSave.onclick=()=>{
      ...
      persistState(); renderExpensesTables(); renderCategories();
      modalOverlay.style.display='none'; showToast('Item updated');
    };
  };

  window.deleteExpense=function(idx){
    if(confirm('Are you sure to delete this item?')){
      state.expenses.splice(idx,1); persistState(); renderExpensesTables(); renderCategories(); showToast('Item deleted');
    }
  };

  btnAdd.addEventListener('click', ()=>{
    modalTitle.textContent='Add Item';
    modalAmount.value=''; modalType.value='expense';
    modalOverlay.style.display='flex';
    modalSave.onclick=()=>{
      const e={category:modalCategory.value,amount:Number(modalAmount.value||0),type:modalType.value,date:new Date().toISOString()};
      state.expenses.push(e); persistState(); renderExpensesTables(); renderCategories();
      modalOverlay.style.display='none'; showToast('Item added');
    };
  });
  ...
  btnSaveLimit.addEventListener('click',()=>{
    const val=Number(inputLimit.value||0); state.profile.limit=val; persistState(); updateProfileUI(); showToast('Limit saved');
  });
  ...
  function showSavings(period){
    ...
    html+='</tbody></table>'; savingsContent.innerHTML=html;
  }
  ...
  // --- Initialize ---
  renderCategories(); renderExpensesTables(); loadProfile();
