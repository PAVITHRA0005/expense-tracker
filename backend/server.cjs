...
const userSchema = new mongoose.Schema({
  ...
  password: String,
  salary: { amount: { type: Number, default: 0 }, type: { type: String, default: 'monthly' } },
...
router.post('/signup', async (req,res)=>{
  try{
    ...
    const user = new User({ username: username || email.split('@')[0], email, password });
    await user.save();
...
router.post('/login', async (req,res)=>{
  try{
    ...
    return res.json({ message: 'Login success', token: fakeToken, userId: user._id, name: user.username, email: user.email });
  }catch(err){
...
router.get('/expenses', async (req,res)=>{
  try{
    ...
    if(!email) return res.status(400).json({ message: 'email required' });
    const user = await User.findOne({ email });
    if(!user) return res.json({ expenses: [], totalExpenses: 0, remaining: 0 });

    const expenses = user.expenses.map(e=>({ _id: e._id, category: e.category, amount: e.amount, type: e.type, date: e.date }));
    const totalExpenses = expenses.reduce((s,e)=>s+(e.type==='expense'?Number(e.amount):0),0);
    const remaining = Math.max(0, Number(user.salary?.amount||0) - totalExpenses);
...
router.post('/expense', async (req,res)=>{
  try{
    ...
    const totalExpenses = user.expenses.reduce((s,e)=>s+(e.type==='expense'?Number(e.amount):0),0);
    const remaining = Math.max(0, Number(user.salary?.amount||0) - totalExpenses);
    const limitExceeded = user.limit && totalExpenses>user.limit;
